...
//	private String prevPrice = null;
	...
	public void loadQuotes(String ticker) {
		...
		//Устанавливаем данные окончания на текущие
...
//				System.out.println(ticker + " : " + date + " : " + adjClose);
			...
			for(int g = size - 1;g >= 0; g--) {
				...
				if(firstTime) {
					...
					int parsedYear = cal.get(Calendar.YEAR) + 1;
					if(parsedYear >= year) {
						return;
					}
					firstTime = false;
			...
			System.out.println("Ошибка при загрузке данных. Для " + ticker);
			return;
