							...
							jScrollPane1.setPreferredSize(new java.awt.Dimension(292, 341));
							{
								TableModel jTable1Model = 
										new DefaultTableModel(
												null,
												new String[] { "Лонг","Spread ratio", "Шорт","Spread ratio" });
								jTable1 = new JTable();
...
//						});
	...
	public void addData(final List<SpreadObject> spreadList) {
		...
		for(int j = 0; j < spreadList.size(); j++) {
			...
			rows[j][1] = firstVal.getAdjClose();
			rows[j][2] = currPair.getSecondStock().getTicker();
			rows[j][3] = secVal.getAdjClose();
		}
		DefaultTableModel model = new DefaultTableModel(rows, new String[] {"Лонг","Spread Ratio","Шорт","Spread Ratio"}){
